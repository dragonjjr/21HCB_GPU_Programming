...
__global__ void removeSeamKernel(const uchar3 *inPixels, int width, int height, uchar3 *outPixels, int newWidth, int *seam) {
    int row = blockIdx.y * blockDim.y + threadIdx.y;
  int col = blockIdx.x * blockDim.y + threadIdx.x;

  if (row < height && col < newWidth){
    if (col < seam[row]){
        outPixels[row * newWidth + col].x = inPixels[row * width + col].x;
        outPixels[row * newWidth + col].y = inPixels[row * width + col].y;
        outPixels[row * newWidth + col].z = inPixels[row * width + col].z;
      }
      else{
        outPixels[row * newWidth + col].x = inPixels[row * width + col + 1].x;
        outPixels[row * newWidth + col].y = inPixels[row * width + col + 1].y;
        outPixels[row * newWidth + col].z = inPixels[row * width + col + 1].z;
      }
  }
}
...
void seamCarvingOnDevice(const uchar3 *inPixels, int width, int height, uchar3 *outPixels, int *xFilter, int *yFilter, int filterWidth, dim3 blockSize){
  ...
  dim3 newGridSize((newWidth - 1) / blockSize.x + 1, (height - 1) / blockSize.y + 1);
	...
	CHECK(cudaMalloc(&d_out, newDataSize));
  ...
  CHECK(cudaMemcpy(outPixels, d_out, newDataSize, cudaMemcpyDeviceToHost));
